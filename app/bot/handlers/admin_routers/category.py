...
from app.schemas.fsm.fsms import AddNewCategory
from app.bot.keyboards.admin_keyboars import (
    access_add_new_category,
...
async def admin_panel_category_toggle(callback: CallbackQuery):
    try:
        ...
        await callback.answer(
            text="❌ Ошибка изменения статуса категории", show_alert=True
        )
        return
...
async def admin_panel_category_add(callback: CallbackQuery, state: FSMContext):
    try:
        ...
        await callback.answer(
            text="❌ Ошибка добавления нового товара", show_alert=True
        )
        return
...
async def process_name_category(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            text="📱 Выберите действие:", reply_markup=access_add_new_category()
        )
