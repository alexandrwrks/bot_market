...
def get_user_basket(): # получаем message: Message для поиска корзины и стоимости корзины
    """Возращаем корзину пользователю по одноу предмету в строку, 
    если у пользователя в корзине есть товары. Проверку делать с помщью БД
    (для тестовых варинатов будем делать корзину сами чтобы проверять функционал)"""
